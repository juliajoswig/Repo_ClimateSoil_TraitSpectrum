...
plot_Figure_1b <- function(origin){
  ...
  if(file.exists(file.path(origin,"data","helper_files","kernel_est","H.RData"))){
    load(file.path(origin,"data","helper_files","kernel_est","H.RData"))
    load(file.path(origin,"data","helper_files","kernel_est","est.RData"))
  }else{
    H <- Hpi(x=sc) # optimal bandwidth estimation
    save(H,file=file.path(origin,"data","helper_files","kernel_est","H.RData"))
    est <- kde(x=sc, H=H, compute.cont=TRUE) # kernel density estimation
    save(est,file=file.path(origin,"data","helper_files","kernel_est","est.RData"))
  ...
  if(!dir.exists(file.path(origin, "figures","figure_1"))){dir.create(file.path(origin, "figures","figure_1"))}
  
{
  pdf(file=file.path(origin, "figures","figure_1","figure_1b_PCA.pdf"),height=14,width=16)
