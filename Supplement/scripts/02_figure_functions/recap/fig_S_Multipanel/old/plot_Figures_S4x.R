plot_Figure_S4x <- function(origin,climOsoil,Appr_type,nruns,fold_now){
  meanOsd="median"
  ...
  if(file.exists(file.path( origin, "data", "VarSelect_output", paste0(nruns,"Reps"), fold_now,
    ...
    for(t in 1:length(trait.names_tmp)){
      pdf(file=file.path(origin,"plots","Fig_S4",
                         paste0(trait.names[t],".pdf")), height=15,width=20)
      ...
      load(file=file.path(origin, "plots","Fig_S4",paste0(trait.names_tmp[t],".RData")))
      ...
      plot(x=c(0,1),y=c(1,1),pch=16,col=c("white"),ylim=c(0,edg),cex=1,#c(min(atm[!is.na(soil)])*1.1,max(atm[!is.na(soil)])*1.1),
           xlim=c(0,edg),axes=FALSE,ylab="",xlab="",frame=FALSE)
      text(x = 0.5, y=.9,labels = "Soil cumulative importance",cex=2)#, srt = -90)
