...
#-----------------------------------------------------
  ...
  if(!file.exists(file.path(origin,"figures","figure_S_Multipanel","Trait_Correl"))){
    dir.create(file.path(origin,"figures","figure_S_Multipanel","Trait_Correl"))
  }
  
  for(trait_now in 1:17){
  
    plot_data <-  c(0,dat_cor_p[trait_now,put_into_traitGroup(colnames(trait1))=="Size"],
                  0,dat_cor_p[trait_now,put_into_traitGroup(colnames(trait1))=="Eco"],
                  0,dat_cor_p[trait_now,put_into_traitGroup(colnames(trait1))=="Other"])
  
    
  pdf(file = file.path(origin,"figures","figure_S_Multipanel","Trait_Correl",paste0(colnames(trait1)[trait_now],"_Correl.pdf")),width=13,height=5)
...
#png(file = file.path(origin,"figures","figure_S_Multipanel","Trait_Correl",paste0(colnames(trait1)[trait_now],"_Correl.png")),width=2500,height=1200,res=300)
#par(mfrow=c(3,8),mar=c(0,0,0,0))
# n=0
# for(i in 1:length(plot_data)){
#   val_now = plot_data[i]
