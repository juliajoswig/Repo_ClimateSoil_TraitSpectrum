fn_SingeTrait_LatGradient <- function(){}
  ...
  require(FactoMineR)
  pca_trait<- PCA(log(trait_agg1_2))
  
  trait_agg1_3<-pca_trait$ind$coord[,1:3]
  ...
  data_trait <-cbind(lat_bins,trait_agg1_3) 
  ...
  t=1
  for(t in 1:ncol(trait_agg1_3)){
  png(file.path(origin, "figures","figure_multipanel_PCA","all",paste0(colnames(trait_agg1_3)[t],"_latgradient.png")),width=800,height=500)
  
  par(mfrow=c(1,1),mar=c(5,5,2,2))
  plot(abs(lat_agg1_2$Lat),data_trait[,t+1],frame.plot = FALSE,ylim=c(-10,10),
       xlim=c(0,110),col=add.alpha(color_to_biomes(info$BIOME),alpha = .5),xaxt="n",
       ylab=paste0(colnames(trait_agg1_3)[t]),
  ...
  points(data_trait_s$Group.1,data_trait_s[,t+1],cex=2.5,
         col="white",pch=16)
  points(data_trait_s$Group.1,data_trait_s[,t+1],cex=1.5,
         col="black",pch=16)
