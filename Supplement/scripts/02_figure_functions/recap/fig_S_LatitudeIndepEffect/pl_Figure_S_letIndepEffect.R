plot_figure_3a <- function(origin){
    ...
    doPCA=TRUE
    output_terms=c("soilLat")
    climOsoils=c("soilAclimate")
    if(!dir.exists(file.path(origin,"figures","figure_S_lat"))){
      dir.create(file.path(origin,"figures","figure_S_lat"))}
...
#    for(output_term in output_terms){
    ...
    #-------------
      ...
      load(file.path(origin, "data", "_results","RidgeRegression","_RidgeRegression_results",
                     paste0("HP_",nruns=2,"nruns_", output_term,pca_term,".RData")))
  ...
  {
    pdf(file = file.path(origin,"figures","figure_S_lat",paste0("figure_S3a_",shorty,".pdf")),width=15,height=15)
