plot_Figure_x <- function(origin,output_term){
  ...
  load(file = file.path(origin,"data","master_matrix","X1"))
  ...
  plot(log(TRY_Env$PlantHeight_gf),log(TRY_Env$SLA_gf),col=color_to_biomes(TRY_Env$BIOME))
  #plot(log(TRY_Env_o2$PlantHeight_gf),log(TRY_Env_o2$SLA_gf),col=color_to_biomes(TRY_Env$BIOME))
  
  plot(log(TRY_Env$ConduitDens_gf),log(TRY_Env$SLA_gf),col=color_to_biomes(TRY_Env$BIOME))
