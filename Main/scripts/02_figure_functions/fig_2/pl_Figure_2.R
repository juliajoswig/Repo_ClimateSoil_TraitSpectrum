plot_Figure_2 <- function(origin){
  ...
  load(file.path(origin,"data","master_matrix","X1.RData")) # species per ecoregion scale
  load(file.path(origin,"data","helper_files","Ecoregions_selected","Ecoregion_Agg2.RData")) # species per ecoregion scale
  X1_gf <- TRY_Env1[TRY_Env1$ECO_ID%in%Ecoregion_Agg2$ECO_ID,]
  X1 <- X1_gf[,colnames(X1_gf)%in%trait_names]
  X1_log <- as.data.frame(log(X1))
  #----------------------------------------------------------------------
  # do PCA
  #----------------------------------------------------------------------
  require(FactoMineR)
  ...
  if(file.exists(file.path(origin,"data","helper_files","kernel_est","H_PC1.RData"))){
    load(file.path(origin,"data","helper_files","kernel_est","H_PC1.RData"))
    load(file.path(origin,"data","helper_files","kernel_est","est_PC1.RData"))
  }else{
    H <- Hpi(x=sc1) # optimal bandwidth estimation
    save(H,file=file.path(origin,"data","helper_files","kernel_est","H_PC1.RData"))
    est <- kde(x=sc, H=H, compute.cont=TRUE) # kernel density estimation
    save(est,file=file.path(origin,"data","helper_files","kernel_est","est_PC1.RData"))
    print("kernel estimation done!")
  }
  # For PC2
  sc2 = cbind(abs(X1_gf$Lat),pca_traits$ind$coord[,2])
  if(file.exists(file.path(origin,"data","helper_files","kernel_est","H_PC2.RData"))){
  }else{
    H <- Hpi(x=sc2) # optimal bandwidth estimation
    save(H,file=file.path(origin,"data","helper_files","kernel_est","H_PC2.RData"))
    est <- kde(x=sc, H=H, compute.cont=TRUE) # kernel density estimation
    save(est,file=file.path(origin,"data","helper_files","kernel_est","est_PC2.RData"))
    print("kernel estimation done!")
  }
  ...
  #----------------------------------------------------------------------------
  { 
    pdf(file=file.path(origin,"figures","figure_2","Figure_2.pdf"),width = 10,height = 8)
    ...
    #----------------------------------------------------------------------------
    load(file.path(origin,"data","helper_files","kernel_est","H_PC2.RData"))
    load(file.path(origin,"data","helper_files","kernel_est","est_PC2.RData"))
    est2=est
    dat1 <- data.frame(sc2)
    names(dat1) <- c("lat","PC")
