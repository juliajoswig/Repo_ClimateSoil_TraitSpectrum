analysis_RidgeRegression <- function(climate,soil,trait,
  ...
  predicted<- list()
    ...
    for(tr in 1:ncol(resp)){
      ...
      p <- matrix(NA,nrow=sum(!is.na(resp[,tr])),ncol=nruns)
      
      chunk <- function(x,n) split(x, factor(sort(rank(x)%%n)))
      chunk_list <- list()
      for(i in 1:nruns){
        chunk_list[[i]] <- chunk(sample(1:sum(!is.na(resp[,tr]))),folds)
      }
      
      for(n in 1:nruns){
        print(paste0("CV run nb ",n,"of ",nruns))
        pred_c <- pred[!is.na(resp[,tr]),]
        resp_c <- resp[!is.na(resp[,tr]),tr]
        
        for(rep in 1:folds){
          
          ix_bt <- !((1:nrow(pred_c))%in%chunk_list[[n]][[rep]])
          
          #internal cross validation to get lambda min for final model.
          cvfit = cv.glmnet(x=as.matrix(pred_c[ix_bt,]), y=as.matrix(resp_c[ix_bt]),nfolds = 10, keep = T)
          
          # predict with final model (defined by lambda min)
          p[!ix_bt,n] = predict(cvfit,newx=as.matrix(pred_c[!ix_bt,]),s="lambda.min")
        } 
        #---------------------------------------------------------------
        # write into output
        #---------------------------------------------------------------
