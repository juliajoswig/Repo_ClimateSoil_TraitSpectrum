...
#-----------------------------------------------------
  trait_now=1
    ...
    plot_data[!is.na(plot_data)] <- "Correl with "
    
  pdf(file = file.path(origin,"figures","figure_S3",paste0("Template_Correl.pdf")),width=13,height=5)
  ...
  for(i in 1:length(plot_data)){
    ...
    if(names(trait1)[trait_now]==names(plot_data)[i]){
      ...
      text(0,.2,labels = "Correl with",pch=16,cex=2,col="black")  
      text(0,0,labels = "resp. trait",pch=16,cex=2,col="black")  
      text(0,0.4,labels=Rename_Vars(names(plot_data)[i])[,3],cex=1.5,col=color_to_traits(names(plot_data)[i]))
      }else{
        if(!is.na(val_now)){points(0,0,pch=16,cex=16,col="gray")}
    text(0,0.4,labels=Rename_Vars(names(plot_data)[i])[,3],cex=1.5,col=color_to_traits(names(plot_data)[i]))
    if(is.na(val_now)){
      n=n+1
