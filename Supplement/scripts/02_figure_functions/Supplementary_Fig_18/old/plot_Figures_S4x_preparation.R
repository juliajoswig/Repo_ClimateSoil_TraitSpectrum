plot_Figures_S4x_preparation <- function(origin,climOsoil,Appr_type,nruns,fold_now){
  
  meanOsd="median"
  
  accuracy_tot <- matrix(NA,ncol=5,nrow=0)
  colnames(accuracy_tot) <- c("trait","r2","meanOsd","climOsoil","subset")
  require('stringr')
  ...
  #------------------------------------------------------------------------------------------------
    ...
    data_now <- data.df
    data.df$relationship <-  rename_what_can_be_renamed(data_now$relationship)
    data.df$relationship <- gsub(x = data.df$relationship,pattern = "_1km",replacement = "")
    ...
    driver_rename <- as.vector(unlist(data.df2$driver))
     ...
     driver_rename <- as.vector(unlist(data.df2$driver))
       ...
       driver_rename[driver_rename%in%c("CECSOL","CECSUM")] <-"CEC"
       driver_rename[driver_rename%in%c("OCDENS")] <-"orgC dens"
     ...
     data.df2$driver2 <-  driver_rename2#rename_for_plot(driver_rename)
     
     #------------------------------------------------------------------------------------------------
     # soil or atm?
     #------------------------------------------------------------------------------------------------
     
     data.df2$soilOatm <- put_into_ClimateOrSoil(data.df2$driver)
     data.df3 <- data.df2
     head(data.df2)
  ...
  l=1
   ...
   for(l in 1:length(unique(data.df4$data.type))){
    ...
    for(l in 1:length(unique(data.df4$driver))){
      ...
      new_list_entry <- list(col = color_to_ClimateSoil(data.df4$soilOatm[which(data.df4$driver%in%new_list_name)[1]]), textcol = "black")
      V_cl_colour$new_list_entry <- new_list_entry
      names(V_cl_colour)[length(V_cl_colour)] <- new_list_name
  ...
  out <- list(RPE=RPE,
              style=style)
  save(out,file=file.path(origin, "plots","Fig_S4",paste0(trait.names[t],".RData")))
