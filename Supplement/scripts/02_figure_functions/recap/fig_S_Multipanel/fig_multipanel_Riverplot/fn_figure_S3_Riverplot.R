Figure_S4_Riverplot <- function(origin){
  ...
  hp_low2 = hp_l[[which(names(hp_l)%in%c("soil_physicsAsoil_chemistry","soil_chemistryAsoil_physics"))]]
  arms=4
...
#    if(new_list_name==nm[1]){color_now="#96ceed"}
  ...
  for(l in 1:length(unique(dat_1$Cat1))){
    ...
    if(new_list_name=="Soil Physics"){color_now=soiltype1_S4}#
    if(new_list_name=="Soil Chemistry"){color_now=soiltype2_S4}#
...
#  plot(1:19,col=rgb(.77, .177, .244),cex=4,pch=16)
  ...
  png(file = file.path(origin,"figures","figure_S4","all",paste0(colnames(hp_l[[1]][[1]])[trait_now],"_River.png")),height=1500,width=2000)
  par(mfrow=c(1,1))
  ...
  for(trait_now in 1:17){
  #  png(file = file.path(origin,"figures","figure_S4",paste0("figure_S4_river_",colnames(hp_l[[1]][[1]])[trait_now],".png")),height=1500,width=2000)
    png(file = file.path(origin,"figures","figure_S4","all",paste0(colnames(hp_l[[1]][[1]])[trait_now],"_River.png")),height=1500,width=2000)
    par(mfrow=c(1,1))
    plot(1:15,frame.plot = FALSE,col="white")
    op = par(cex = 4.1,no.readonly = TRUE)
