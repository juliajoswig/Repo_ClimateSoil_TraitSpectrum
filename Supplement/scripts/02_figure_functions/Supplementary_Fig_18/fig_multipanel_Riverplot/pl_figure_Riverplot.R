plot_figure_S4_Riverplot <- function(origin){
  ...
  if(!file.exists(file.path(origin,"figures","Supplement_Fig_18_33","Riverplot"))){
    dir.create(file.path(origin,"figures","Supplement_Fig_18_33","Riverplot"))
  }
...
#  plot(1:19,col=rgb(.77, .177, .244),cex=4,pch=16)
  ...
  save(RPE_l,file=file.path(origin,"figures","Supplement_Fig_18_33","Riverplot",paste0("Riverplot_info.RData")))

  # load riverplot
  load(file.path(origin,"figures","Supplement_Fig_18_33","Riverplot",paste0("Riverplot_info.RData")))
  ...
  #  png(file = file.path(origin,"figures","figure_S4",paste0("figure_S4_river_",colnames(hp_l[[1]][[1]])[trait_now],".png")),height=1500,width=2000)
    try(dev.off())
    png(file = file.path(origin,"figures","Supplement_Fig_18_33","Riverplot",paste0(colnames(hp_l[[1]][[1]])[trait_now],"_River.png")),height=1500,width=2000)
    par(mfrow=c(1,1))
