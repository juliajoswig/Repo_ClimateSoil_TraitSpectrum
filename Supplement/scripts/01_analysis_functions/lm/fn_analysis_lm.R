analysis_lm <- function(climate,soil,trait,
  ...
  pred_pca <- cbind(pca_clim,pca_soil)
  pred <- as.data.frame(pred_pca)

  if(output_term=="PCA"){response <- trait}else{
    response <- log(trait)}
  resp <- response
  ...
  for(tr in 1:ncol(resp)){
    ...
    save(out, file = file.path(origin, "data", "_results",type_analysis,climOsoil, paste0(nruns,"Reps"),paste0("Res_",output_term,pca_term,".RData")))
