pl_multiplot_S4x <- function(){
  ...
  require(FactoMineR)
  pca_trait<- PCA(log(trait1))
  PCA_trait <- pca_trait$ind$coord[,1:3]
  trait2=cbind(log(trait1),PCA_trait)
  trait_cor <- cor(trait2,method = "pearson")
...
#-----------------------------------------------------
  ...
  if(!dir.exists(file.path(origin,"figures","figure_multipanel_PCA","all"))){
   ...
   trait_now=20
  for(trait_now in 18:20){
  
    plot_data <-  c(NA,dat_cor_p[trait_now,put_into_traitGroup(colnames(trait2))=="Size"],
                    NA,dat_cor_p[trait_now,put_into_traitGroup(colnames(trait2))=="Eco"],
                    NA,dat_cor_p[trait_now,put_into_traitGroup(colnames(trait2))=="Other"])
  
  pdf(file = file.path(origin,"figures","figure_multipanel_PCA","all",paste0(colnames(trait2)[trait_now],"_Correl.pdf")),width=13,height=5)
  ...
  for(i in 1:length(plot_data)){
    ...
    if(names(trait2)[trait_now]==names(plot_data)[i]){
