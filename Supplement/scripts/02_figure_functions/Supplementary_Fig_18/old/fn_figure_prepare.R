...
Figure_S4_prepare_S4 <- function(origin,trait){
  
  load(file.path(origin, "data", "_results","RidgeRegression","hp_RR",paste0("HP_",nruns,"nruns_",Appr_type_now, output_term,".RData")))
  load(file.path(origin, "data", "_results","RidgeRegression","hp_RR",paste0("R2_",nruns,"nruns_",Appr_type_now, output_term,".RData")))
...
#  dat_2b$Value[2] <- r2_2 #climate value 
  ...
  for(l in 1:length(unique(dat_1$Cat2))){
    ...
    if(new_list_name=="climate"){color_now="#96ceed"}#
    if(new_list_name=="soil"){color_now="#bb82ba"}#
  ...
  for(l in 1:length(unique(dat_1$Cat1))){
    ...
    if(new_list_name=="energy"){color_now="#96ceed"}#"ffe474"
    if(new_list_name=="water"){color_now="#96ceed"}#
    if(new_list_name=="soil physics"){color_now="#bb82ba"}#
    if(new_list_name=="soil chemistry"){color_now="#bb82ba"}#
    if(new_list_name=="climate"){color_now="#96ceed"}#
    if(new_list_name=="soil"){color_now="#bb82ba"}#
    V_cl_colour[[new_list_name]] <- list(col = color_now, textcol = "black")
  }  
...
#  plot(1:19,col=rgb(.77, .177, .244),cex=4,pch=16)
  ...
  save(RPE_l,file=file.path(origin,"figures","figure_S4",paste0("Riverplot_info.RData")))

  # load riverplot
  load(file.path(origin,"figures","figure_S4",paste0("Riverplot_info.RData")))
