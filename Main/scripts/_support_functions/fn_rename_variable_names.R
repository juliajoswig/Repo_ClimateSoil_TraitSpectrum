Rename_Vars <- function(input){
  ...
  #traits 2/2
  transl_t[which(transl_t[,1]%in%"LeArea"),2] <- "Leaf area" 
  transl_t[which(transl_t[,1]%in%"Led15N"),2] <- "Leaf d15N"
  transl_t[which(transl_t[,1]%in%"LeNArea"),2] <- "Leaf N area"
  transl_t[which(transl_t[,1]%in%"LeNP"),2] <- "Leaf N:P"
  transl_t[which(transl_t[,1]%in%"LeC"),2] <- "Leaf C"
  transl_t[which(transl_t[,1]%in%"LeP"),2] <- "Leaf P" 
  transl_t[which(transl_t[,1]%in%"LeN"),2] <- "Leaf N" 
  transl_t[which(transl_t[,1]%in%"SLA"),2] <- "SLA" 
  transl_t[which(transl_t[,1]%in%"ConduitDens"),2] <- "Conduit dens" 
  transl_t[which(transl_t[,1]%in%"LeFMass"),2] <- "Leaf f mass" 
  transl_t[which(transl_t[,1]%in%"SenbU"),2] <- "Seeds U" 
  transl_t[which(transl_t[,1]%in%"PlantHeight"),2] <- "Height" 
  transl_t[which(transl_t[,1]%in%"SeLen"),2] <- "Seed len" 
  transl_t[which(transl_t[,1]%in%"VesLen"),2] <- "Vessel len" 
  transl_t[which(transl_t[,1]%in%"DispULen"),2] <- "Dispersal len" 
  transl_t[which(transl_t[,1]%in%"SeedMass"),2] <- "Seed mass"  
  transl_t[which(transl_t[,1]%in%"SSD"),2] <- "Stem dens" 
